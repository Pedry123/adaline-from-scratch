...
def animate(i, data, weights_history, title):
    ...
    #plotline(data, current_weights, f'{title}')
...
class Perceptron:
    ...
    def train(self, inputs, outputs, weights, num_samples):
        epoch = 0
        ...
        while True:
            ...
            for k in range(num_samples):
                prediction = np.dot(weights, inputs[:, k])
                y = 1 if prediction >= 0 else -1
                if y != outputs[k]:
                    weights = weights + self.learning_rate * (outputs[k] - y) * inputs[:, k]
                    error = True
            self.weights_history.append(weights.copy())
            self.errors_list.append(error)
            epoch += 1
            print(f'Epoch: {epoch}, Error: {error}, Weights: {weights.reshape(-1)}') # o reshape foi feito apenas para melhorar a visualização
            if not error:
                break
...
if __name__ == "__main__":
    ...
    data, outputs = make_blobs(n_samples=40, centers=2, n_features=2, random_state=167)
    ...
    perceptron = Perceptron(0.045, 1e-6)
    weights = perceptron.train(inputs, outputs, weights, inputs.shape[1])

    # Criar animação
    fig = plt.figure(figsize=(10, 6))
    ani = FuncAnimation(fig, animate, frames=range(0, len(perceptron.weights_history), 1),
                        fargs=(data, perceptron.weights_history, 'Perceptron'),
                        interval=10000, blit=False, repeat=False)
